...
class QuranService {
  ...
  static List<Sura> suraSearchResults = List.generate(114, ((index) => getSuraFromIndex(index)));

  static List<Sura> mostRecentlySuras = [];

  static Sura getSuraFromIndex(int index) => Sura(nameEN: suraNameEN[index],
...
num: index+1);
  ...
  static void searchSura(String query){
    ...
    for(int i =0;i<114;i++){
      if(suraNameAR[i].contains(query) || suraNameEN[i].toLowerCase().contains(query.toLowerCase())){
  ...
  static Future<void> addSuraToMostRecently(Sura sura) async{
    if(mostRecentlySuras.contains(sura)) {
      mostRecentlySuras.remove(sura);
    }
